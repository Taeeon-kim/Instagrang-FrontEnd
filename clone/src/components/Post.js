    ...
    <React.Fragment>
      <Grid border="1px solid #DBDBDB" position  margin="auto" width="50%" >
        ...
        <Grid is_flex padding="6px 0px 8px 10px">
         {result.length ===1 || is_like?  <IconButton likeIcon padding="8px" _onClick={()=>{setLike(!is_like); dispatch(postAction.likePost(props.postId))}  }/> : <IconButton unLikeIcon padding="8px" _onClick={()=>{setLike(!is_like); dispatch(postAction.likePost(props.postId))}}/> } <IconButton commentIcon padding="8px"/>   {/*좋아요하트 아이콘, 댓글말풍선 아이콘*/} 
        </Grid>
        <Grid>
